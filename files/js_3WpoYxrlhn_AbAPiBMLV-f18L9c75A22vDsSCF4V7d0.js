...
(function($, Drupal) {
    var scrollDown = function(e) {
        var menuscroll = $('.bar-top--styleguide');
        var bartitle = $('.bar-top__title--styleguide');
        var wS = $(window).scrollTop();
        var sticky = 300;
        if ((wS > sticky)) {
            menuscroll.addClass('bar-top__down--styleguide');
            bartitle.addClass('bar-top__title__down--styleguide');
        } else {
            menuscroll.removeClass('bar-top__down--styleguide');
            bartitle.removeClass('bar-top__title__down--styleguide');
        }
    };
    ...
    Drupal.behaviors.menuScrollCEEALBA = {
        attach: function(context, settings) {
            var menuscroll = $('.navigation--ceealba');
            $(window).scroll(function() {
                var c = $(this).scrollTop();
                if ((c > 80)) menuscroll.addClass("menu__scroll");
                else menuscroll.removeClass("menu__scroll");
            });
        }
    };
    ...
    function showMenu() {
        $(".menu-main__level1").removeClass("show-menu");
    }

    function fondoHeader() {
        if (!$('.page-node-type-landing-page')[0]) {
            $('.fondo').attr('srcset', '/eadt/sites/eadt/themes/espai_eadt/assets/images/portada_subpages/12_mg_8235_0.jpg');
            return;
        }
        let min = 0;
        let max = 3;
        let random = Math.floor(Math.random() * (max - min + 1)) + min;
        let img1 = 'file/fotografia.png';
        let img2 = 'files/fotografia.png';
        let img3 = 'files/fotografia.png';
        let img4 = 'files/fotografia.png';
        let imgHeader = new Array(img1, img2, img3, img4);
        $('.fondo').attr('srcset', imgHeader[random]);
    }

    function slider() {
        $("#ca84a783").hover(function() {
            showMenu();
            $("#ca84a783 .menu-main__level1").first().addClass('show-menu');
        }, function() {
            showMenu();
        });
        $("#d596546c").hover(function() {
            showMenu();
            $("#d596546c .menu-main__level1").first().addClass('show-menu');
        }, function() {
            showMenu();
        });
        $("#e7794135").hover(function() {
            showMenu();
            $("#e7794135 .menu-main__level1").first().addClass('show-menu');
        }, function() {
            showMenu();
        });
        $("#62d1af1d").hover(function() {
            showMenu();
            $("#62d1af1d .menu-main__level1").first().addClass('show-menu');
        }, function() {
            showMenu();
        });
    }
    ...
    $(document).ready(function() {
        ...
        $('.more__btn').click(function() {
            let search = $('.search-content-form');
...
(function(Drupal) {
    ...
    Drupal.behaviors.cookiesGa = {
        consentGiven: function() {
            var scriptIds = ['cookies_ga_ga', 'cookies_ga_ga_debug', 'cookies_ga_google_analytics_tracking_script', 'cookies_ga_google_analytics_tracking_file'];
            for (var scriptId in scriptIds) {
                ...
                if (script) {
                    ...
                    for (var attr in attributes) {
                        ...
                        if (name !== 'type' && name !== 'id') newScript.setAttribute(name, attributes[attr].nodeValue);
                    }
        ...
        attach: function(context) {
            ...
            document.addEventListener('cookiesjsrUserConsent', function(event) {
                ...
                if (typeof service.analytics !== 'undefined' && service.analytics) self.consentGiven(context);
            });
        }
    };
})(Drupal);;;
(function(Drupal) {
    ...
    Drupal.behaviors.cookiesGtag = {
        ...
        attach: function(context) {
            ...
            document.addEventListener('cookiesjsrUserConsent', function(event) {
                var service = (typeof event.detail.services === 'object') ? event.detail.services : {};
                if (typeof service.gtag !== 'undefined' && service.gtag) self.consentGiven(context);
            });
        }
    };
})(Drupal);;